...
export const getAllMetaData = (userData = null, isFromInitial = false) => {
    return (dispatch, getState) => {
        let token = "";
        if(userData){
            token = "Bearer "+userData.token
        }else{
            token = "Bearer "+getState().user.token
        }
        return CallApi(Constant.baseUrlV2+Constant.metaData,'get',{},{"Authorization":token})
            .then((response)=> {
                try{
